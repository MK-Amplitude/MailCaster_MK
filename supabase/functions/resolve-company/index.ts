...
사용자가 입력한 회사명/약어/브랜드명과 (옵션) 이메일 도메인을 받아, 다음 5가지를 JSON 으로 반환합니다:
  - name_ko: 한글 정식 명칭
  - name_en: 영문 정식 명칭
  - parent_group: 한국 기업집단(그룹) 명. 계열사를 거느린 그룹의 일원이거나 그룹 대표/지주 법인 자체면 그룹명. 계열사 없는 단일 독립기업만 null
...
4) parent_group (그룹사) — 한국 기업집단명. 한글로 짧게.
   ★ 정책: 자회사·계열사뿐 아니라 "여러 계열사를 거느린 그룹의 대표/지주 법인 자체" 도 그 그룹명으로 표기.
      즉 그 회사가 기업집단(그룹)의 일원이면 — 지주사든 자회사든 대표 법인이든 — 그룹명을 채운다.
      계열사가 없는 진짜 단일 독립기업만 parent_group=null.
   - 그룹 대표법인 자체도 표기: raw="kt" → "KT", raw="비바리퍼블리카" → "토스",
     raw="주식회사 카카오" → "카카오", raw="네이버(주)" → "네이버", raw="SK주식회사" → "SK".
   - 알려진 그룹사(이에 한정되지 않음): 롯데, 신세계, 삼성, 현대, 현대백화점, LG, SK, GS, 한화, CJ,
     카카오, 네이버, KT, 두산, 코오롱, 효성, 포스코, 농심, 오리온, 미래에셋, 골프존, 이랜드, SPC, 하림,
     빙그레, 종근당, 한미약품, 셀트리온, 넥슨, 엔씨소프트, 넷마블, 크래프톤, NHN, 토스(비바리퍼블리카),
     쿠팡, 우아한형제들, 야놀자, 컴투스, 스마일게이트, 하이브, HD현대, KB금융, 신한금융, 하나금융, 우리금융, 등
   - 매핑 예시:
       "kt" / "주식회사 KT" / "KT Corporation" / "KT엠모바일" / "KT클라우드" / "kt알파" / "BC카드" / "케이뱅크" / "지니뮤직" / "나스미디어" → parent_group="KT"
       "비바리퍼블리카" / "토스" / "토스뱅크" / "토스증권" / "토스페이먼츠" / "토스인슈어런스" → parent_group="토스"
       "신세계아이앤씨" / "SHINSEGAE I&C" / "신세계디에프" / "신세계푸드" / "신세계까사" / "신세계프라퍼티" → parent_group="신세계"
       "쿠팡" / "쿠팡페이" / "쿠팡플레이" / "쿠팡이츠" / "쿠팡풀필먼트" → parent_group="쿠팡"
   ...
   - 계열사 없는 단일 독립기업 / 식별 불가 → parent_group=null
