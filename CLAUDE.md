...
- **현재 최신 마이그레이션: `077_drop_dead_columns.sql`**
- 신규 마이그레이션 파일명은 `078_` 부터 시작 (반드시 migrations/ 디렉터리의 실제 최신 순번을 먼저 확인할 것)
...
supabase/
├── migrations/         # 001_~077_ SQL 마이그레이션 (순번 관리)
├── functions/          # 16개(_shared 포함) Deno Edge Functions
